...
## Intro
If you're trying to get this app up and running on your end:
1. Fork and clone the repository.
2. In the root directory, run the following commands in order:
   - A) `pipenv install && pipenv shell`: Installs project dependencies, creating the `Pipfile.lock`. Activates the project's virtual environment.
   - B) `npm install --prefix client`: Installs dependencies for the frontend located in the `package.json` file in the `client` subdirectory. Creates `package-lock.json`.
   - C) `python server/app.py`: Starts the app and logs all server responses in this terminal.
3. Open a new terminal and run this command:
   - A) `npm start --prefix client`: Starts the frontend, opening a new tab in your web browser. If it doesn't, navigate to `localhost:4000` in your web browser.
